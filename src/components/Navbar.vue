<template>
  <div id="navbar">
    ...
    <v-toolbar class="cyan darken-1 hidden-xs-only">
      <v-toolbar-title class="font-weight-medium"> MSPi</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-toolbar-items v-if="user.loggedIn">
        <!-- if user is logged in !-->
        <v-btn text v-for="(item, i) in navLoggedIn" :key="i" :to="item.route">
          {{ item.text }}
        </v-btn>
      ...
      <v-toolbar-items v-else>
        ...
        <v-btn text v-for="(item, i) in navLoggedOut" :key="i" :to="item.route">
          {{ item.text }}
        </v-btn>
      </v-toolbar-items>
    </v-toolbar>
